...
- If the user asks to create/edit/delete nodes inside Figma itself, switch to the Cursor Figma MCP `figma-use` skill.
- If the user asks to build screens from the `tmp/manda/` prototype (no Figma), switch to `.cursor/skills/claude-design-implementation/SKILL.md`.
- For Manda-specific tokens and primitives, follow `.cursor/rules/architecture.mdc` (§9) and reuse `components/manda/*`.
